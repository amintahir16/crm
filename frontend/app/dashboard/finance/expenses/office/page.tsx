...
export default function OfficeExpensesPage() {
  ...
  const [modalType, setModalType] = useState<'office' | 'rent'>('office');
  ...
  const fetchExpenses = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Filter for both office and rent
        const filtered = data.expenses.filter((e: any) => 
          e.category === 'office' || e.category === 'office_rent'
        );
  ...
  const fetchTrend = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/expenses/trend/office`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/expenses`, {
        ...
        body: JSON.stringify({
          ...
          category: modalType === 'office' ? 'office' : 'office_rent',
          expenseName: modalType === 'rent' ? `Office Rent - ${new Date(formData.expenseDate).toLocaleString('default', { month: 'long', year: 'numeric' })}` : formData.expenseName
        }),
      });
  ...
  const totalOffice = expenses
    .filter(e => e.category === 'office' && new Date(e.expenseDate).getFullYear() === currentYear)
    .reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0);
  const totalRent = expenses
    .filter(e => e.category === 'office_rent' && new Date(e.expenseDate).getFullYear() === currentYear)
    .reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => { setModalType('office'); setShowModal(true); }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-300">
              <p className="text-sm font-medium text-gray-500 mb-1">Total Office Expenses</p>
              <p className="text-2xl font-bold text-gray-900">{formatPKR(totalOffice)}</p>
              <div className="mt-4 flex items-center text-xs text-blue-600 font-semibold uppercase tracking-wider">
                  ...
                  <tr key={e.id} className="hover:bg-gray-50/80 transition-colors group">
                    ...
                    <td className="px-6 py-5">
                      <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider ${
                        e.category === 'office_rent' ? 'bg-indigo-50 text-indigo-600' : 'bg-blue-50 text-blue-600'
                      }`}>
                        {e.category === 'office_rent' ? 'Rent' : 'Daily'}
                      </span>
                    </td>
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900">
                {modalType === 'office' ? 'Add Office Expense' : 'Record Office Rent'}
              </h2>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              {modalType === 'office' && (
                <div>
                  ...
                  <input 
                    required
                    type="text"
                    placeholder="e.g. Stationary, Internet Bill"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    type="number"
                    placeholder="0.00"
                    className="w-full px-4 py-3 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-blue-500 text-sm font-bold"
              ...
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Notes / Details</label>
                <textarea 
                  rows={3}
                  placeholder="Additional information..."
                  className="w-full px-4 py-3 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-blue-500 text-sm resize-none"
